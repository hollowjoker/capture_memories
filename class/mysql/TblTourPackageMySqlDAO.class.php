...
class TblTourPackageMySqlDAO implements TblTourPackageDAO{
	...
	public function insert($tblTourPackage){
		$sql = 'INSERT INTO tbl_tour_package (name, travel_period_from_at, travel_period_to_at, selling_period, image_path, status, created_at, updated_at, deleted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
		$sqlQuery = new SqlQuery($sql);
	...
	public function update($tblTourPackage){
		$sql = 'UPDATE tbl_tour_package SET name = ?, travel_period_from_at = ?, travel_period_to_at = ?, selling_period = ?, image_path = ?, status = ?, created_at = ?, updated_at = ?, deleted_at = ? WHERE id = ?';
		$sqlQuery = new SqlQuery($sql);
